...
import com.dtstack.taier.spark.common.constant.ConfigConstant
...
import org.apache.spark.deploy.yarn.DtConfig.{KRB5FILENAME, KRB5_CONF}
...
private[spark] class DtClient(
                             ...
                             val hadoopConfstr: Configuration,
...
//  private val hadoopConf = new YarnConfiguration(SparkHadoopUtil.newConfiguration(sparkConf))
  ...
  private val executorMemoryOverhead = sparkConf.get(EXECUTOR_MEMORY_OVERHEAD).getOrElse(
    math.max((0.1 * executorMemory).toLong,
      ResourceProfile.MEMORY_OVERHEAD_MIN_MIB)).toInt
  ...
  private val krb5 = sparkConf.get(KRB5_CONF).orNull
  private val krb5FileName = KRB5FILENAME
  ...
  def prepareLocalResources(
    ...
    val hadoopconfpath: String = sparkConf.get(ConfigConstant.SPARK_HADOOP_CONF_REMOTE_DIR)
    distribute(hadoopconfpath, destName = Some(SparkConstants.HADOOP_CONF))

    sparkConf.getOption(DtConfig.SPARK_UDFS_TO_DISTRIBUTE.key) match {
      case Some(v) => v.split(",").foreach(
        lib => {
          val libPath = lib.split("/").last
          distribute(lib, destName = Some(libPath), appMasterOnly = false)
        }
      )
    ...
    amKeytabFileName.foreach { kt =>
      ...
      val remoteKeytab: String = sparkConf.get(ConfigConstant.SPARK_KERBEROS_REMOTE_KEYTAB)
      val (_, localizedPath) = distribute(remoteKeytab, destName = Some(kt), appMasterOnly = true)
    ...
    amKeytabFileName.foreach { kt =>
      logInfo("To enable the AM to login from keytab, credentials are being copied over to the AM" +
        " via the YARN Secure Distributed Cache.")
      val (_, localizedPath) = distribute(krb5,
        destName = Some(krb5FileName))
      require(localizedPath != null, "Keytab file already distributed.")
    }
    ...
    if (sparkConf.contains("spark.submit.pyFiles")) {
      sparkConf.get("spark.submit.pyFiles").split(",").toSeq.foreach { f =>
    ...
    // distributed file.
    amKeytabFileName.foreach { kt => confsToOverride.put("spark.yarn.keytab", kt) }
    amKeytabFileName.foreach { kt => confsToOverride.put("spark.kerberos.keytab", kt) }
  ...
  private def createConfArchive(confsToOverride: Map[String, String]): File = {
    ...
    try {
      ...
      confStream.putNextEntry(new ZipEntry("log4j.properties"))
      val sparkLog4jContent: String = sparkConf.get("spark.log4j.content")
      confStream.write(sparkLog4jContent.getBytes(Charset.forName("UTF-8")))
  ...
  private def setupLaunchEnv(
    ...
    if (sparkConf.contains("spark.submit.pyFiles")) {
      val (pyFiles, pyArchives) = sparkConf.get("spark.submit.pyFiles").split(",").toSeq.partition(_.endsWith(".py"))
      if (pyFiles.nonEmpty) {
  ...
  private def startApplicationMasterService(report: ApplicationReport): ApplicationMaster = {
    ...
    val amService = new Thread("Unmanaged Application Master Service") {
      override def run(): Unit = {
        appMaster.runUnmanaged(null, report.getCurrentApplicationAttemptId,
...
private object DtClient extends Logging {
  ...
  private[yarn] def populateClasspath(
    ...
    addClasspathEntry(
      buildPath(Environment.PWD.$$(), LOCALIZED_HADOOP_CONF_DIR), env)
