...
private[spark] class DtClient(
  ...
  private var krb5: String = null
  private var krb5FileName = KRB5FILENAME
  ...
  def setupCredentials(): Unit = {
    ...
    if (loginFromKeytab) {
      ...
      krb5 = sparkConf.get(KRB5_CONF).orNull
  ...
  def prepareLocalResources(
    ...
    if (sparkConf.get("security").equalsIgnoreCase("true")){
      ...
      nearestTimeOfNextRenewal = credentialManager.obtainCredentials(hadoopConf, credentials)
    }
    ...
    val hadoopConfPath: String = sparkConf.get(ConfigConstant.SPARK_HADOOP_CONF_REMOTE_DIR).toString
    ...
    if (loginFromKeytab) {
      logInfo("To enable the AM to login from krb5.conf, credentials are being copied over to the AM" +
        " via the YARN Secure Distributed Cache.")
      val (_, localizedPath) = distribute(krb5,
        destName = Some(krb5FileName))
      require(localizedPath != null, "krb5.conf file already distributed.")
    }
    ...
    val localConfArcFile = createConfArchive()
    val localConfArchive = new Path(localConfArcFile.toURI())
    try {
      copyFileToRemote(destDir, localConfArchive, replication, symlinkCache, force = true,
        destName = Some(LOCALIZED_CONF_ARCHIVE))
    }finally {
      if(!localConfArcFile.delete) {
        logError("failed to delete file " + localConfArcFile.getAbsolutePath)
      }
    }
...
//    new DtClient(args, sparkConf, yarnClient = yarnClient).run()
  }
