	...
	export default {
        ...
        createSession(params: any) {
			// 创建会话
            return http.post(req.CREATE_SESSION, params);
        },
        closeSession(sessionHandle: string) {
			// 关闭会话
            return http.delete(req.CLOSE_SESSION(sessionHandle));
        },
        getRowSetResult(operationHandle: string, params?: any) {
			// 获取结果
            return http.get(req.GET_ROW_SET_RESULT(operationHandle), { params });
        },
        executeStatement(sessionHandle: string, params: any) {
			// 执行语句
            return http.post(req.EXECUTE_STATEMENT(sessionHandle), params);
        },
        operationClose(sessionHandle: string, params: any) {
			// 关闭operation
            return http.put(req.OPERATION_CLOSE(sessionHandle), params);
        },
        operationCancel(sessionHandle: string, params: any) {
			// 取消operation
            return http.put(req.OPERATION_CANCEL(sessionHandle), params);
        },
        getAllSessions(params?: any) {
			// 获取所有会话
            return http.get(req.GET_ALL_SESSIONS, { params });
        },
	};
